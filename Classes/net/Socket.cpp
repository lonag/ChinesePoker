...
#endif		
		if((clientfd = (SOCKET)TEMP_FAILURE_RETRY(::accept(fd,&sa,&addrlen))) == INVAILD_FD){
			return;
		}
...
void Socket::onReadAct()
{
	if(state == listening)
		...
		int n = (int)TEMP_FAILURE_RETRY(::recv(fd,recvbuf,recvbuf_size,0));
		if(n == 0){
			Close();
#if (CC_TARGET_PLATFORM == CC_PLATFORM_WIN32)				
...
int  Socket::rawSend(){
	while(writeable && !sendlist.empty()){
		...
		size_t len = wpk.Size()-wpos+sizeof(int);
		int n = (int)TEMP_FAILURE_RETRY(::send(fd,&buf[wpos],len,0));
		if(n == 0){
			//Close();
			writeable = false;
			return -1;
