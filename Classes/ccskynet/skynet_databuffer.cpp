...
void ByteBuffer::put(const char* bytes,int offset,int len){
	if(position + len > capacity){
		log("error -ByteBuffer::put(const char* bytes,int offset,int len)---position=%d,len=%d,capacity=%d\n",position,len,capacity);
		return;
	}
...
void ByteBuffer::putByte(byte n){
	if(position + 1 > capacity){
		log("error ByteBuffer::putByte----position=%d,len=%d,capacity=%d\n",position,1,capacity);
		return;
	}
	buffer[position] = n;
	position++;
...
void ByteBuffer::put(int n){
	if(position + 1 > capacity){
		log("error ByteBuffer::put---position=%d,len=%d,capacity=%d\n",position,1,capacity);
		return;
	}
...
void ByteBuffer::putInt(int n){
	if(position + 4 > capacity){
		log("error ByteBuffer::putInt--position=%d,len=%d,capacity=%d\n",position,4,capacity);
		return;
	}
	for(int i = 0 ; i < 4 ; i++){
...
void ByteBuffer::setIntAt(int n,int index){
	if(index + 4 > capacity){
		log("error ByteBuffer::setIntAt--index=%d,len=%d,capacity=%d\n",index,4,capacity);
		return;
	}
	for(int i = 0 ; i < 4 ; i++){
...
void ByteBuffer::putFloat(float n){
	if(position + 4 > capacity){
		log("error -ByteBuffer::putFloat---position=%d,len=%d,capacity=%d\n",position,4,capacity);
		return;
	}
...
void ByteBuffer::putShort(short n){
	if(position + 2 > capacity){
		log("error -ByteBuffer::putShort---position=%d,len=%d,capacity=%d\n",position,2,capacity);
		return;
	...
	log("buffer short = %s  %d",byteToHexStr((unsigned char*)buffer,2).c_str(),n);
}

void ByteBuffer::putLong(long long n){
	if(position + 8 > capacity){
		log("error ByteBuffer::putLong-position=%d,len=%d,capacity=%d\n",position,8,capacity);
		return;
	}
...
void ByteBuffer::putUTF(const char* str){
	...
	if(position + 2+len > capacity){
		log("error ByteBuffer::putUTF----position=%d,len=%d,capacity=%d\n",position,2+len,capacity);
		return;
	}
	putShort(len);
	if( len>0){
...
void ByteBuffer::setPosition(int p){
	if(p > limit) {
		log("error ByteBuffer::setPosition p> limit------------p=%d,limit=%d\n",p,limit);
	}
	position = p;
}
...
char ByteBuffer::get(){
	...
	log("error ByteBuffer::get() position+1> limit------------position=%d,limit=%d\n",position,limit);
	return 0;	
}
byte ByteBuffer::getByte(){
	if(position + 1 > limit){
		log("error ByteBuffer::getByte() position+1> limit------------position=%d,limit=%d\n",position,limit);
		return 0;
	}
	return buffer[position++];
}
bool ByteBuffer::getBoolean(){
	if(position + 1 > limit){
		log("error ByteBuffer::getBoolean() position+1> limit------------position=%d,limit=%d\n",position,limit);
		return false;
	}
...
int ByteBuffer::getInt(){
	if(position + 4 > limit){
		log("error ByteBuffer::getInt() position+4> limit------------position=%d,limit=%d\n",position,limit);
		return 0;
	}
	int rt = 0;
	for(int i = 0 ; i < 4 ; i++){
...
float ByteBuffer::getFloat(){
	if(position + 4 > limit){
		log("error ByteBuffer::getFloat() position+4> limit------------position=%d,limit=%d\n",position,limit);
		return 0;
	}
	float floatValue;
	unsigned char* pBytes;
...
short ByteBuffer::getShort(){
	if(position + 2 > limit){
		log("error ByteBuffer::getShort() position+2> limit------------position=%d,limit=%d\n",position,limit);
		return 0;
	}
	short n = 0;
	for(int i = 0 ; i < 2 ; i++){
...
long long ByteBuffer::getLong(){
	if( position + 8 > limit ){
		...
		log("error ByteBuffer::getLong() position+8> limit------------position=%d,limit=%d\n",position,limit);
		return 0;
...
void ByteBuffer::get(char* bytes,int offset,int len){
	if(position + len > limit){
		...
		log("error ByteBuffer::get(char* bytes,int offset,int len) position+len> limit------------position=%d,len=%d,limit=%d\n",position,len,limit);
		return;
	}
...
{
	if( position+1> capacity ){
		log("error putBoolean position+len> limit------------position=%d,len=%d,capacity=%d\n",position,1,capacity);
