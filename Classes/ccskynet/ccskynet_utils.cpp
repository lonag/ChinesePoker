#include "../ccskynet/ccskynet_utils.h"
#include <stdlib.h>
...
bool writeSaveData(const char* relatePath,char* buf,int len){
	...
	if( fp!=NULL){
		...
		if( nn!= len){
			printf("writeFileData(%s) error write  size(%d) < len(%d) \n ",pszfullFilepath.c_str(),(int)nn,len);
			return false;
		}
		return true;
	}
...
ByteBuffer* readSaveData(const char* relatePath){	
	...
	do 
	{
		...
		int ret = (int)fread(pBuffer->getBuffer(), size, 1,fp);
		if( ret!=1){
			delete pBuffer;
			pBuffer=NULL;
		}
...
//	printf("Util:: getFileData() %s,data = %p,size = %lld\n ",pszFileName,pBuffer,size);
	ByteBuffer* p = NULL;
	if( size>0 && pBuffer){
		p =  new ByteBuffer(pBuffer,0,(int)size);
	}
...
unsigned char* getFileData(const char* pszFileName, bool isfullPath,unsigned int* size)
{
	...
	*size = (unsigned int)lsize;
    //printf("Util:: getFileData() %s,data = %p,size = %u\n ",pszFileName,pBuffer,size);
